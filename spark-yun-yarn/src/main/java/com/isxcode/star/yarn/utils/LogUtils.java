...
public class LogUtils {
  ...
  public static Map<String, String> parseYarnLog(String applicationId) {
    ...
    if (Strings.isEmpty(yarnConfig.get("yarn.resourcemanager.webapp.address"))) {
      throw new RuntimeException("请在yarn-site.xml中配置yarn.resourcemanager.webapp.address属性:${yarn.resourcemanager.hostname}:8088");
    }

    // 访问yarn作业日志页面
    Map appInfoMap = new RestTemplate().getForObject("http://" + yarnConfig.get("yarn.resourcemanager.webapp.address") + "/ws/v1/cluster/apps/" + applicationId, Map.class);
    ...
    if (Strings.isEmpty(jobHistoryAddress)) {
      throw new RuntimeException("请在mapred-site.xml中配置mapreduce.jobhistory.webapp.address属性:0.0.0.0:19888");
    }
    ...
    for (Element element : preElements) {
      ...
      if (!logUrl.isEmpty()) {
        try {
          logStr = Jsoup.connect("http://" + jobHistoryAddress + logUrl).get().body().getElementsByTag("pre").text();
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
