...
public class SparkJarExecutor extends WorkExecutor {
    ...
    protected String execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance,
        ...
        // 获取作业运行上下文
        SparkJarExecutorContext workEventBody =
            JSON.parseObject(workEvent.getEventContext(), SparkJarExecutorContext.class);
        if (workEventBody == null) {
            workEventBody = new SparkJarExecutorContext();
        ...
        if (processNeverRun(workEvent, 1)) {
            ...
            logBuilder.append(LocalDateTime.now()).append(WorkLog.SUCCESS_INFO).append("作业配置检测完成 \n");
            workInstance = updateInstance(workInstance, logBuilder);

            // 保存配置信息
            workEventBody.setJarJobConfig(jarJobConfig);
            workEventBody.setJarFile(jarFile);
        ...
        if (processNeverRun(workEvent, 3)) {
            ...
            // 获取配置信息
            JarJobConfig jarJobConfig = workEventBody.getJarJobConfig();
            FileEntity jarFile = workEventBody.getJarFile();
            ClusterEntity clusterEntity = workEventBody.getClusterEntity();
            ...
            executeReq.setClusterType(clusterEntity.getClusterType());
            ...
            // 保存作业配置信息
            workEventBody.setExecuteReq(executeReq);
            workEvent.setEventContext(JSON.toJSONString(workEventBody));
            workEventRepository.saveAndFlush(workEvent);
        }
        ...
        if (processNeverRun(workEvent, 4)) {
            ...
            // 保存appId信息
            workEventBody.setSubmitWorkRes(submitWorkRes);
            workEvent.setEventContext(JSON.toJSONString(workEventBody));
            workEventRepository.saveAndFlush(workEvent);
        }
        ...
        if (processNeverRun(workEvent, 5)) {
            ...
            // 获取作业信息
            RunWorkRes submitWorkRes = workEventBody.getSubmitWorkRes();
            SubmitWorkReq executeReq = workEventBody.getExecuteReq();
            ClusterEntity clusterEntity = workEventBody.getClusterEntity();
            ClusterNodeEntity engineNode = workEventBody.getEngineNode();

            // 提交作业成功后，开始循环判断状态
            String oldStatus = workEventBody.getCurrentStatus() != null ? workEventBody.getCurrentStatus() : "";
            while (true) {
                ...
                if (runningStatus.contains(workStatusRes.getAppStatus().toUpperCase())) {
                    // 保存当前状态
                    workEventBody.setCurrentStatus(oldStatus);
                    workEventBody.setWorkStatusRes(workStatusRes);
                    workEvent.setEventContext(JSON.toJSONString(workEventBody));
                    workEventRepository.saveAndFlush(workEvent);
        ...
        if (processNeverRun(workEvent, 6)) {
            ...
            // 获取作业信息
            RunWorkRes submitWorkRes = workEventBody.getSubmitWorkRes();
            RunWorkRes workStatusRes = workEventBody.getWorkStatusRes();
            SubmitWorkReq executeReq = workEventBody.getExecuteReq();
            ClusterEntity clusterEntity = workEventBody.getClusterEntity();
            ClusterNodeEntity engineNode = workEventBody.getEngineNode();
            ...
            if (successStatus.contains(workStatusRes.getAppStatus().toUpperCase())) {
                ...
                // 获取数据
                GetWorkStdoutLogReq getWorkStdoutLogReq = GetWorkStdoutLogReq.builder().appId(submitWorkRes.getAppId())
                    .clusterType(clusterEntity.getClusterType()).sparkHomePath(executeReq.getSparkHomePath()).build();
                baseResponse = HttpUtils.doPost(httpUrlUtils.genHttpUrl(engineNode.getHost(), engineNode.getAgentPort(),
                ...
                // 如果是k8s类型，需要删除k8s容器
                if (AgentType.K8S.equals(clusterEntity.getClusterType())) {
    ...
    public static class SparkJarExecutorContext {
        ...
        private RunWorkRes submitWorkRes;

        private RunWorkRes workStatusRes;
