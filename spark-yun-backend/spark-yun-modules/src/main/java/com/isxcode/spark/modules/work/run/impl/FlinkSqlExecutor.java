...
//
// public FlinkSqlExecutor(WorkInstanceRepository workInstanceRepository, ClusterRepository
// clusterRepository,
...
// workRunJobFactory,
// VipWorkVersionRepository vipWorkVersionRepository, SqlFunctionService sqlFunctionService,
// SecretKeyRepository secretKeyRepository, SqlValueService sqlValueService) {
//
...
// clusterNodeRepository.findAllByClusterIdAndStatus(engineOpt.get().getId(),
...
// for (SecretKeyEntity k : secretKeyRepository.findAll()) {
// script = script.replace("${{ secret." + k.getKeyName() + " }}", k.getSecretValue());
// printSql = printSql.replace("${{ secret." + k.getKeyName() + " }}", "******");
// }
...
// submitJobReq.setAgentHomePath(engineNode.getAgentHomePath() + "/" +
...
// PluginReq pluginReq = PluginReq.builder().sql(workEventBody.getScript()).build();
// if (workRunContext.getFuncConfig() != null) {
// List<FuncEntity> allFunc = funcRepository.findAllById(workRunContext.getFuncConfig());
...
// }
// getJobInfoRes = JSON.parseObject(JSON.toJSONString(baseResponse.getData()),
// GetWorkInfoRes.class);
...
// workInstance = updateInstance(workInstance, logBuilder);
//
// List<String> running = Arrays.asList("RUNNING", "UNDEFINED", "SUBMITTED", "CONTAINERCREATING",
// "PENDING",
...
// GetWorkLogReq.builder().agentHomePath(engineNode.getAgentHomePath() + "/" +
// PathConstants.AGENT_PATH_NAME)
...
// FlinkAgentUrl.GET_WORK_LOG_URL),
...
// GetWorkLogRes getWorkLogRes = JSON.parseObject(JSON.toJSONString(baseResponse.getData()),
// GetWorkLogRes.class);
