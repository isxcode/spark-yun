...
public class ExcelSyncExecutor extends WorkExecutor {
    ...
    public ExcelSyncExecutor(WorkInstanceRepository workInstanceRepository, ClusterRepository clusterRepository,
        ...
        FileService fileService, Scheduler scheduler, WorkEventRepository workEventRepository,
        WorkRunJobFactory workRunJobFactory, VipWorkVersionRepository vipWorkVersionRepository, WorkService workService,
        SecretKeyRepository secretKeyRepository, DataSourceFactory dataSourceFactory,
    ...
    protected String execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance,
        ...
        if (workEvent.getEventProcess() == 2) {
            ...
            if (!Strings.isEmpty(workRunContext.getExcelSyncConfig().getQueryCondition())) {
                ...
                // 保存事件
                workRunContext.getExcelSyncConfig().setQueryCondition(script);
        ...
        if (workEvent.getEventProcess() == 5) {
            ...
            if (workRunContext.getExcelSyncConfig().isFileReplace()) {
                ...
                if (!fileEntityOptional.isPresent()) {
                    throw errorLogException(fileName + "不存在");
                }
            ...
            // 上传csv文件
            try {
                scpJar(scpNodeInfo, csvFilePath,
                    agentNode.getAgentHomePath() + "/zhiqingyun-agent/file/" + csvFileName);
                // 删除本地的csv文件
                FileUtil.del(csvFilePath);
            } catch (JSchException | SftpException | InterruptedException | IOException ex) {
                FileUtil.del(csvFilePath);
                throw errorLogException("excel文件上传失败");
            }

            // 保存事件
            workRunContext.setCsvFileName(csvFileName);

            // 保存日志
            logBuilder.append(endLog("上传同步文件完成"));
        ...
        if (workEvent.getEventProcess() == 6) {
            ...
            ClusterNodeEntity agentNode = workRunContext.getAgentNode();
            String csvFileName = workRunContext.getCsvFileName();

            // 构建请求体
            SubmitWorkReq submitWorkReq = new SubmitWorkReq();
            submitWorkReq.setWorkId(workRunContext.getWorkId());
            submitWorkReq.setWorkType(WorkType.EXCEL_SYNC_JDBC);
            ...
            PluginReq pluginReq = PluginReq.builder().excelSyncConfig(workRunContext.getExcelSyncConfig())
                ...
                .syncRule(workRunContext.getSyncRule()).agentType(clusterType).build();
            pluginReq.setCsvFilePath(agentNode.getAgentHomePath() + File.separator + "zhiqingyun-agent" + File.separator
        ...
        // 清理作业执行文件
        if (workEvent.getEventProcess() == 10) {
