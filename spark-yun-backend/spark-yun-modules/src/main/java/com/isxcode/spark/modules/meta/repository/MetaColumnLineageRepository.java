...
public interface MetaColumnLineageRepository extends JpaRepository<MetaColumnLineageEntity, String> {
    ...
    @Query("select new com.isxcode.spark.api.meta.ao.MetaColumnLinageAo(M.id, M.fromDbId, M.fromTableName,M.fromColumnName, M.toDbId, M.toTableName,M.fromColumnName,D.dbType,D.name,D.remark,M.remark,M.workVersionId,M.workId,W.name,W.workType,W.remark,WF.name,WF.remark) from MetaColumnLineageEntity M left join DatasourceEntity D on M.fromDbId = D.id left join WorkEntity W on M.workId = W.id left join WorkflowEntity WF on W.workflowId=WF.id where M.toDbId = :toDbId and M.toTableName = :toTableName and M.toColumnName = :toColumnName and M.tenantId = :tenantId ")
    ...
    @Query("select new com.isxcode.spark.api.meta.ao.MetaColumnLinageAo(M.id, M.fromDbId, M.fromTableName,M.fromColumnName, M.toDbId, M.toTableName,M.fromColumnName,D.dbType,D.name,D.remark,M.remark,M.workVersionId,M.workId,W.name,W.workType,W.remark,WF.name,WF.remark) from MetaColumnLineageEntity M left join DatasourceEntity D on M.toDbId = D.id left join WorkEntity W on M.workId = W.id left join WorkflowEntity WF on W.workflowId=WF.id where M.fromDbId = :fromDbId and M.fromTableName = :fromTableName and M.fromColumnName = :fromColumnName and M.tenantId = :tenantId ")
