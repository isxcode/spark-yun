...
public interface MetaDatabaseRepository extends JpaRepository<MetaDatabaseEntity, String> {
    ...
    @Query("SELECT M FROM MetaDatabaseEntity M WHERE M.dbName LIKE %:keyword% OR M.dbComment LIKE %:keyword% order by  M.status asc ,M.createDateTime desc,M.dbName asc")
