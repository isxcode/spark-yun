   ...
   public boolean checkoutKafka() {
    String bootstrapServers = "localhost:9092";
		...
		try (AdminClient adminClient = AdminClient.create(properties)) {
			...
			// 判断特定的主题是否存在
			String topicToCheck = "ispong-topic";
			if (topics.contains(topicToCheck)) {
				System.out.println("成功连接到 Kafka 主题: " + topicToCheck);
			} else {
				System.out.println("无法连接到 Kafka 主题: " + topicToCheck);
			}
		} catch (InterruptedException | ExecutionException e) {
			System.out.println("连接到 Kafka 主题时发生错误: " + e.getMessage());
		}
		return true;
	}

	public void checkKafka(KafkaConfig kafkaConfig) {
