...
public class FileBizService {
	...
	public FileEntity fileUpload(MultipartFile file, String type) throws IOException {

		// 文件存储路径
		String localPath = sparkYunProperties.getResourceDir() + File.separator + type + File.separator
		...
		return fileRepository.save(FileEntity.builder().fileName(file.getOriginalFilename()).fileType(type)
				.filePath(localPath).fileSize(getDataSize(file.getSize())).build());
	}
