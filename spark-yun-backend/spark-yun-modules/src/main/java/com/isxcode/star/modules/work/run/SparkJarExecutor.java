...
public class SparkJarExecutor extends WorkExecutor {
  ...
  protected void execute(WorkRunContext workRunContext, WorkInstanceEntity workInstance) {
    ...
    try {
      ...
      throw new WorkRunException(LocalDateTime.now() + WorkLog.ERROR_INFO + "传输jar失败 : " + e.getMessage() + "  \n");
    }
    if (sparkJarConfigEntity.getLibFileIds() != null) {
      sparkJarConfigEntity.getLibFileIds().forEach(fileId ->
        ...
        try {
          ...
          throw new WorkRunException(LocalDateTime.now() + WorkLog.ERROR_INFO + "传输lib失败 : " + e.getMessage() + "  \n");
        }
      }));
    }
