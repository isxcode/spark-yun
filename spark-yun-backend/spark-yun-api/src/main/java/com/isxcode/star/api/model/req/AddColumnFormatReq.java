...
@Data
public class AddColumnFormatReq {

    @Schema(title = "数据标准名称", example = "123")
    @NotEmpty(message = "name不能为空")
    private String name;

    @Schema(title = "数据标准字段类型编码", example = "TEXT|STRING|DATE|DOUBLE|CUSTOM|INT|DATETIME")
    @NotEmpty(message = "columnTypeCode不能为空")
    @Pattern(regexp = "^(TEXT|STRING|DATE|DOUBLE|CUSTOM|INT|DATETIME)$",
        message = "只可以输入TEXT|STRING|DATE|DOUBLE|CUSTOM|INT|DATETIME")
