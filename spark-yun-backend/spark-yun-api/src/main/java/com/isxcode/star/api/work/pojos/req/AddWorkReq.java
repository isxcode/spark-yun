...
public class AddWorkReq {
	...
	@Schema(title = "作业类型", example = "类型编码：" + "SPARK_SQL： spark sql执行作业" + "EXE_JDBC： jdbc执行作业"
			...
			+ "API： 接口调用作业" + "PRQL： PRQL查询作业" + "CURL： CURL作业")
	@NotEmpty(message = "作业类型不能为空")
	@Pattern(regexp = "^(SPARK_SQL|EXE_JDBC|QUERY_JDBC|DATA_SYNC_JDBC|BASH|PYTHON|SPARK_JAR|SPARK_CONTAINER_SQL|API|PRQL|CURL)$", message = "作业类型不支持")
