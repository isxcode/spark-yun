...
> ModuleCode：模块编码
> 一个模块编码对应一个controller，且在项目开发过程中属于统一话术标准

编辑文件文件：`spark-yun/spark-yun-backend/spark-yun-api/src/main/java/com/isxcode/spark/api/main/constants/ModuleCode.java`
 ...
 * Demo案例模块.
 */
String EXAMPLE_DEMO = "example_demo";
```

#### 2. 创建SQL脚本

> 基于Flyway管理Sql，目前仅支持h2和mysql和postgresql数据库

编辑文件：`spark-yun/spark-yun-backend/spark-yun-main/src/main/resouces/db/migration/h2/V30__GH-88.sql`
编辑文件：`spark-yun/spark-yun-backend/spark-yun-main/src/main/resouces/db/migration/mysql/V30__GH-88.sql`
编辑文件：`spark-yun/spark-yun-backend/spark-yun-main/src/main/resouces/db/migration/postgres/V30__GH-88.sql`

- 要求1：文件名称`V「顺序写编号」__「Github Issue编号」`, `V`一定要大写，**下划线一定要两个**，例如GH-88,则写成 `V30__GH-88.sql`
- 要求2：建表中要包含系统字段，如create_by、create_date_time、last_modified_by、last_modified_date_time、version_number、deleted、tenant_id
- 要求3：`SY_「xxx」`(SY为spark_yun的缩写) XXX结尾不需要加S、表名全部大写、用下划线分割  

```sql
create table if not exists SY_EXAMPLE_DEMO
(
    id                       varchar(200)  not null unique primary key comment '唯一id',
    demo_name                varchar(200)  not null comment '名称',
    demo_content             varchar(200)  null comment '内容',
...
> 基于Jpa实现对象持久化，支持Jpa乐观锁、假删除、多租户、基础系统字段
> 实体需要与数据库中的表一一对映

编辑文件：`spark-yun/spark-yun-backend/spark-yun-modules/src/main/java/com/isxcode/spark/modules/example/demo/entity/ExampleDemoEntity.java`

- 要求1：一定在entity目录下创建文件，modules/example/demo/entity
- 要求2：文件名「XXX」Entity.java
- 要求3：目录需要根据下划线分为多个目录，比如`EXAMPLE_DEMO`，则在`example/demo/entity`目录下

```java
@Data
@Entity
@SQLDelete(sql = "UPDATE SY_EXAMPLE_DEMO SET deleted = 1 WHERE id = ? and version_number = ?")
@Where(clause = "deleted = 0 ${TENANT_FILTER} ")
@Table(name = "SY_EXAMPLE_DEMO")
@JsonIgnoreProperties({"hibernateLazyInitializer"})
@EntityListeners(AuditingEntityListener.class)
public class ExampleDemoEntity {
...
> 基于Jpa对数据库进行互动，对数据库进行增删改查

编辑文件 `spark-yun/spark-yun-backend/spark-yun-modules/src/main/java/com/isxcode/spark/modules/example/demo/repository/ExampleDemoRepository.java`   

- 要求1：一定在repository目录下创建文件，modules/example/demo/repository
- 要求2：文件名「XXX」Repository.java
- 要求3：不要超过三张表Join
...
```java
@Repository
@CacheConfig(cacheNames = {ModuleCode.EXAMPLE_DEMO})
public interface ExampleDemoRepository extends JpaRepository<ExampleDemoEntity, String> {
    ...
    // 自定义更新，非特殊情况禁止使用
    @Modifying
    @Query(value = "update ExampleDemoEntity set content = :content where name = :name")
    void updateContentByName(@Param("content") String content, @Param("name") String name);

    // 对象分页搜索查询
    @Query("SELECT D FROM ExampleDemoEntity D WHERE D.name LIKE %:searchKeyWord% order by D.createDateTime desc")
...
> 多表关联查询返回对象「XXX」AO.java, Association Object联合对象

- 要求1：注意，一定要写构造器，不能使用lombok   
- 要求2：jpa写法，数据库字段与构造器中的参数顺序保持一致
- 要求3：最好不要超过三张表Join关系 
- 要求4：一定在ao目录下创建文件，`spark-yun/spark-yun-backend/spark-yun-api/src/main/java/com/isxcode/spark/api/example/demo/ao`
- 要求5：文件名「XXX」Ao.java

```java
@Data
public class ExampleInfoAo {

  private String username;

  private String content;

  public ExampleInfoAo(String username,String content) {
...
@Query(value =
  "select "
    + " new com.isxcode.spark.api.instance.ao.WorkflowInstanceAo("
    + "   U.account,"
    + "   E.content) "
    + "from ExampleDemoEntity E left join UserEntity U on D.id = U.id "
    + "where U.username LIKE %:keyword% AND E.tenantId=:tenantId order by D.createDateTime desc")
Page<ExampleInfoAo> pageExampleInfoAo(
