...
#### 1. 创建ModuleCode，创建后端模块编码

> 一个模块对应一个controller，且在项目开发过程中属于统一话术标准

- 要求：变量名大写、变量值小写、下划线分割、最好一个单词

编辑：`spark-yun-backend/spark-yun-api/src/main/java/com/isxcode/spark/api/main/constants/ModuleCode.java`
...
#### 2. 创建SQL脚本，创建模块需要使用的表

> 基于Flyway管理Sql，目前仅支持h2和mysql和postgresql数据库

- 要求1：脚本命名规范：`V「顺序写编号」__「Github Issue编号」`,  V一定要大写，下划线一定要两个，例如GH-88,则需要写成 `V30__GH-88`
- 要求2：字段包含一下系统字段
- 要求3：SY_XXX  (SY为spark_yun的缩写) XXX结尾不需要加S、表名全部大写、用下划线分割  

编辑：`spark-yun-main/src/main/resouces/db/migration/h2/V30__GH-88.sql`
编辑：`spark-yun-main/src/main/resouces/db/migration/mysql/V30__GH-88.sql`
编辑：`spark-yun-main/src/main/resouces/db/migration/postgres/V30__GH-88.sql`
...
#### 3. 创建Entity，创建表对应的Java实体对象 


> 基于Jpa持久化对象，与数据库中的表一一对应，支持Jpa乐观锁、假删除、多租户、基础系统字段

- 要求：目录需要根据下划线分为多个目录，比如 EXAMPLE_DEMO，则是 example/demo/entity 目录下

编辑：`com.isxcode.spark.modules.example.demo.entity.ExampleDemoEntity`
...
#### 4. 创建Repository，创建数据库互动层 

> 基于Jpa对数据库持久化操作   
> 

编辑 `com.isxcode.spark.modules.<module_code>.repository.<module_code>Repository`   
