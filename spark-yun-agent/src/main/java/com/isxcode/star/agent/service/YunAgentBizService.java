...
public class YunAgentBizService {
  ...
  public ContainerGetDataRes executeContainerSql(ExecuteContainerSqlReq executeContainerSqlReq) {
    ...
    ContainerGetDataReq containerGetDataReq = ContainerGetDataReq.builder().sql(executeContainerSqlReq.getSql()).build();

    try {
      ResponseEntity<ContainerGetDataRes> forEntity = new RestTemplate().postForEntity(
        "http://127.0.0.1:" + executeContainerSqlReq.getPort() + "/getData", containerGetDataReq, ContainerGetDataRes.class);
      return forEntity.getBody();
    } catch (Exception e) {
