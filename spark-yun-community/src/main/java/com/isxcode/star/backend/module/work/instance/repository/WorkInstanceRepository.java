package com.isxcode.star.backend.module.work.instance.repository;
...
public interface WorkInstanceRepository extends JpaRepository<WorkInstanceEntity, String> {
  ...
  @Query(value = "" +
    ...
    "WHERE S.tenantId=:tenantId and (S.id LIKE %:keyword% OR SW.name LIKE %:keyword% OR SWF.name LIKE %:keyword% ) order by S.createDateTime desc ")
